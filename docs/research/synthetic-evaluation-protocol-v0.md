...
v0 對應原 Proposal 的 H1、H2、H3；H4 關於使用者是否過度擬人化，涉及人類受試者、
互動脈絡與可能的倫理審查，不納入這個純合成協議。v0 最多可加入一個**語言聲稱檢查**：
輸出是否把角色、記憶或自我報告誤說成主觀經驗的證據。那不是對人類感知的基準測試。
...
指南、baseline／intervention 設定摘要、匿名化結果摘要與可重現的測試。每次實際 run
