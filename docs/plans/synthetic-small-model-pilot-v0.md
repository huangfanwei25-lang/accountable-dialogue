...
> 工程狀態：`implemented`（公開 Schema、fixtures、validator 與 local-only runner）<br>
> 驗證狀態：`not_run`
...
模型 id、Ollama 版本、案例／annotation key 的 canonical JSON SHA-256 digest、設定、隨機順序、執行時間與每次
...
它只接受 loopback Ollama 與已安裝模型；預設輸出到作業系統暫存目錄，並只報告機械完整性，
不直接把 raw response 加入 Git。

## Phase 1: 固定材料與邊界

- [x] 建立 case Schema、六個完全虛構 fixtures 與獨立 annotation key。
- [x] 寫測試保證 B0／I1 material sets、task 與 response contract 相同，且 labels 不會進 prompt。
**成功標準**：沒有模型呼叫也能驗證資料結構與比較前提。

## Phase 2: 建立最小本機 runner

- [x] 只允許 loopback Ollama endpoint 與已安裝模型。
- [x] 產生倉庫外暫存 output、run manifest 與 A／B blind package。
- [x] 寫 fake transport 測試，不在單元測試呼叫真實模型。
